    ...
    <div className="w-full py-6 bg-white">
      <div className="max-w-screen-xl mx-auto px-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-md text-red-500 cursor-pointer hover:underline font-medium" onClick={() => navigate("/movies")}>
            See All
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-col-4 xl:grid-cols-5 gap-4">
          {data?.map((movie, index) => {
            return <Card key={index} movie={movie} index={index} handleNavigate={handleNavigate} />;
          })}
        </div>
      </div>
    </div>
